...
describe('api client settings step-3b methods (System / Providers / Workspace tabs)', () => {
  ...
  it('providersConfig GETs /settings/{project}/providers/config', async () => {
    const fetchFn = mockFetchOnce({
      ...
      store_connected: true,
      providers: [
        { name: 'kaidera-manifold', label: 'Kaidera AI Manifold', key_is_set: true, is_custom: false, testable: true, provider_ref: 'kaidera_manifold_api_key', key_field: 'kaidera_manifold_api_key', project_id: 'project-1' },
      ],
    })
    ...
    expect(out.providers[0].provider_ref).toBe('kaidera_manifold_api_key')
  })
  ...
  it('providerKeyTest POSTs the probe body to /provider-key-test', async () => {
    ...
    await api.providerKeyTest('kaidera-os', { provider: 'kaidera_manifold_api_key', use_stored: true })
    const { path, body } = lastPost(fetchFn)
    expect(path).toBe('/settings/kaidera-os/provider-key-test')
    expect(body).toEqual({ provider: 'kaidera_manifold_api_key', use_stored: true })
  })
