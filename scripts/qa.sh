...
ruff check .
mypy --ignore-missing-imports --follow-imports=skip --check-untyped-defs \
...
python3 -m compileall -q .agents/api .agents/scripts local-cortex/console/app \
...
PYTHONPATH=.agents/api python3 -m pytest -q .agents/tests
python3 -m pytest -q scripts/fitness/tests
